...
import { MOCK_PORTFOLIO, MOCK_HOLDINGS, MOCK_TRANSACTIONS, MOCK_WATCHLIST } from '@/lib/mock-data'
      ...
      <GlassCard className="col-span-8 p-5">
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-lg font-semibold">{formatCurrency(p.totalValue)}</p>
          </div>
      ...
      <div className="col-span-4 flex flex-col gap-4">
        <GlassCard className="p-5">
          ...
          <div className="flex items-end gap-2">
            <p className={`text-2xl font-bold ${p.totalGain >= 0 ? 'text-gain' : 'text-loss'}`}>
              {p.totalGain >= 0 ? '+' : ''}{formatCurrency(p.totalGain)}
            </p>
            <span className={`text-sm font-medium mb-0.5 ${p.totalGain >= 0 ? 'text-gain' : 'text-loss'}`}>
              {formatPercent(p.totalGainPercent)}
            </span>
          </div>
          <div className="mt-3 h-1.5 rounded-full bg-white/[0.06] overflow-hidden">
            <div
              className="h-full rounded-full bg-gradient-to-r from-accent to-gain"
              style={{ width: `${Math.min((p.totalValue / p.initialBalance) * 50, 100)}%` }}
            />
          </div>
          <div className="flex justify-between mt-2 text-[10px] text-text-muted">
            <span>Initial: {formatCurrency(p.initialBalance)}</span>
            <span>Current: {formatCurrency(p.totalValue)}</span>
          </div>
        </GlassCard>
        ...
        <div className="space-y-1">
          ...
          {MOCK_HOLDINGS.slice(0, 5).map((h) => {
            ...
            return (
              <div
                key={h.ticker}
                className="grid grid-cols-6 gap-4 px-3 py-3 rounded-xl hover:bg-white/[0.03] transition-colors cursor-pointer"
              >
                <div className="col-span-2 flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-accent-bg border border-accent/10 flex items-center justify-center text-[10px] font-bold text-accent">
                    {h.ticker.slice(0, 2)}
                  </div>
                  <div>
                ...
                <div className="text-right self-center">
                  ...
                  <p className={`text-[11px] ${h.change >= 0 ? 'text-gain' : 'text-loss'}`}>
                    {h.change >= 0 ? '+' : ''}{h.change.toFixed(2)}
                  </p>
                </div>
                <p className="text-sm font-medium text-right self-center">{formatCurrency(value)}</p>
                <div className="text-right self-center">
                  <p className={`text-sm font-medium ${isUp ? 'text-gain' : 'text-loss'}`}>
        ...
        <GlassCard className="p-5">
          ...
          <div className="space-y-2">
            {MOCK_WATCHLIST.map((item) => (
              <div
                key={item.ticker}
                className="flex items-center justify-between py-2 px-2 rounded-lg hover:bg-white/[0.03] transition-colors cursor-pointer"
              >
                <div>
